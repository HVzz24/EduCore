<?php
// Defensive: ensure $rows is an array to avoid PHP Notice when it's null
if (empty($rows) || ! is_array($rows)) {
	...
	$tgl_posting   = ($rows['tgl_posting'] != '') ? tgl_indo($rows['tgl_posting']) : '';
	$tgl_mulai   = ($rows['tgl_mulai'] != '') ? tgl_indo($rows['tgl_mulai']) : '';
	$tgl_selesai = ($rows['tgl_selesai'] != '') ? tgl_indo($rows['tgl_selesai']) : '';
	$isi_agenda = nl2br($rows['isi_agenda']);
	$baca = (int) $rows['dibaca'] + 1;
?>
			...
			<div class="block">
				...
				<div class="block-content">
					<div class="shortcode-content">
						...
						if ($ia && is_array($ia)) {
							echo "<a href='".(isset($ia['url']) ? $ia['url'] : '#')."' target='_blank'>";
								$string = isset($ia['gambar']) ? $ia['gambar'] : '';
								if ($string != ''){
									if(preg_match("/swf\z/i", $string)) {
										...
										echo "<img style='margin-top:-10px; margin-bottom:5px' width='100%' src='".base_url()."asset/foto_iklantengah/".$ia['gambar']."' title='".(isset($ia['judul'])?$ia['judul']:'')."' />";
									}
								}
							echo "</a>";
						}
